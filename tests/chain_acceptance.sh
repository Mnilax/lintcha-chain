...
pages.sort(); const expected = [\"404.html\", \"deployer/index.html\", \"hold/index.html\", \"index.html\", \"live/index.html\"]; console.log(\"pages emitted: \" + pages.join(\" \")); console.log(\"language directories: \" + (fs.existsSync(path.join(root, \"es\")) || fs.existsSync(path.join(root, \"pt\")) ? \"present\" : \"none\")); process.exit(JSON.stringify(pages) === JSON.stringify(expected) ? 0 : 1);'"
