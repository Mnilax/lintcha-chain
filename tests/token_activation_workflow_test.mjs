// Static security contract for the manual token activation workflow. This test never invokes git, GitHub, an RPC endpoint or
...
const file = path.join(root, ".github", "workflows", "token-activation.yml");
...
ok(/^name: token-activation$/m.test(code), "the workflow has the stable token-activation name");
...
ok(/^concurrency:\n  group: token-activation\n  cancel-in-progress: false$/m.test(code), "concurrent token activation runs serialize without cancellation");
...
ok(!/gh pr merge|--auto|git push origin main|wrangler|deploy|release|git tag/i.test(review), "token activation neither merges nor deploys, releases or tags anything");
ok(!prep.includes("GITHUB_OUTPUT") && !activation.includes("GITHUB_OUTPUT") && !/(TOKEN_CA|TOKEN_PONS_URL|LINTCHA_CHAIN_RPC_URL)[^\n]*GITHUB_OUTPUT|GITHUB_OUTPUT[^\n]*(TOKEN_CA|TOKEN_PONS_URL|LINTCHA_CHAIN_RPC_URL)/.test(code), "no public input or RPC setting reaches a workflow output");
...
console.log(`token-activation workflow: ${checks} checks, ${failures} failure(s)`);
