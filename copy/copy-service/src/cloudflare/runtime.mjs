...
import { D1AuditSink, D1IntentStore, D1KillSwitchStore, D1OutboxStore, D1ReplayStore, D1SpendLedger, D1UserStore } from "./d1-store.mjs";
...
export async function buildRuntime(env, { coordinated = true, fetchImpl = globalThis.fetch, clock = () => Math.floor(Date.now() / 1000) } = {}) {
  ...
  const localService = new LintchaCopyService({ policyGate, simulator: new SimulationQuorum({ rpcPool, maxGasEstimateSkewBps: config.rpc.maxGasEstimateSkewBps }), rpcPool, auditLog, intentStore: new D1IntentStore(db), confirmationSecret, clock });
  ...
  const surface = new CopyTelegramSurface({ userStore, killSwitches, appOrigin: config.appOrigin, appPath: config.appPath, service });
  const handler = createCopyHttpHandler({
    config, service, surface, userStore, outbox, killSwitches, clock,
  ...
  return Object.freeze({ config, service, localService, handler, killSwitches, userStore, outbox, rpcPool });
}
