...
2. **RPC accounts**: create Alchemy and QuickNode Robinhood Mainnet endpoints, approve any paid plans, and enter both URLs directly into the Copy Cloudflare secrets. Then run `copy-service/tools/rpc-acceptance.mjs`; never paste the URLs into chat or Git.
...
7. **Public activation**: only after the preceding evidence, apply DNS/TLS and the Worker route for `https://copy.lintcha.com`, enable the Core service binding, deploy the current Core Worker, and verify health/outbox/webhook end to end while the global kill switch remains paused.
