...
3. **Copy origin**: the public homepage and Copy-first navigation live on `https://lintcha.com/`; the secure trading sheet uses the selected isolated origin `https://copy.lintcha.com/`. Create its DNS/TLS/Worker route. Do not serve the signing sheet from the Core origin.
