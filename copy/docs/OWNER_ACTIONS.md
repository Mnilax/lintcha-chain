...
3. **Privy application**: create the production Privy app, enable Telegram login and user-owned embedded wallets, create a P-256 server authorization key in the executor secret store, and define a policy that permits only the deployed `LintchaPonsAutoBuy.buy(address,uint256,uint256)` call on chain 4663. Do not grant arbitrary-call, export or root permission to the signer.
